...
def location_and_time_data(file_name='location_time', use_cache=True):
    ...
    if use_cache and os.path.exists(cache_path):
        ...
        # To get review json
        line_count = len(open(os.path.join(EXCEL_PATH, "review.json"), encoding='utf8').readlines())
        user_ids, business_ids, stars, dates = [], [], [], []
        with open(os.path.join(EXCEL_PATH, "review.json"), encoding='utf8') as f:
            for line in tqdm(f, total=line_count):
                blob = json.loads(line)
                user_ids += [blob["user_id"]]
                business_ids += [blob["business_id"]]
        ...
        line_count = len(open(os.path.join(EXCEL_PATH, "business.json"), encoding='utf8').readlines())
        name, business_id, address, city, state, postal_code = [], [], [], [], [], []
        latitude, longitude, stars, review_count = [], [], [], []
        is_open, attributes, GoodForKids, categories, hours = [], [], [], [], []

        with open(os.path.join(EXCEL_PATH, "business.json"), encoding='utf8') as f:
            for line in tqdm(f, total=line_count):
                blob = json.loads(line)
                name += [blob["name"]]
        ...
        business = pd.DataFrame(
            {"name": name, "business_id": business_id, "address": address,
        ...
        detail_df = pd.merge(left=ratings, right=business, on='business_id',
                             how='left')
        mean_lat = detail_df.groupby('user_id')[
            'latitude'].mean().reset_index()
        mean_long = detail_df.groupby('user_id')[
        ...
        detail_df['distance'] = ((detail_df['mean_lat'] - detail_df[
            'latitude']) ** 2) + ((detail_df['mean_long'] - detail_df[
            'longitude']) ** 2)
        ...
        # e(1/1+dist)/e
        # e(1/ log(date))/e
        detail_df['dist_scale'] = np.exp(1 / (1 + detail_df['distance'])) / (
            np.exp(1))
        detail_df['date_scale'] = np.exp(
            1 / (1 + np.log(detail_df['date_diff'] + 1))) / np.exp(1)

        # Multiplying rating scale with the dist
        detail_df['date_rating'] = detail_df['rating'] * detail_df[
            'date_scale']
        detail_df['dist_rating'] = detail_df['rating'] * detail_df[
            'dist_scale']
...
def all_data(file_name='all_data', use_cache=True):
    ...
    if use_cache and os.path.exists(cache_path):
        ...
        line_count = len(open(os.path.join(EXCEL_PATH, "user.json"), encoding='utf8').readlines())
        ...
        user = pd.DataFrame(
            {   "name": name, "user_id": user_id, "review_count": review_count,
                "yelping_since": yelping_since, "useful": useful,
                "funny": funny, "cool": cool, "elite": elite,
                "fans": fans, "average_stars": average_stars,
        ...
        line_count = len(open(os.path.join(EXCEL_PATH, "tip.json"), encoding='utf8').readlines())
        ...
        tip = pd.DataFrame({"business_id": business_id, "user_id": user_id, "text": text,
        ...
        temp = pd.merge(temp, tip, on=['business_id', 'user_id', 'date'],
                        how='left')
